...
返回：成功则返回FD_OK；失败则返回如下值：
	  ...
	  PIM_ERROR_EXCEED_MAX_SESSION_COUNT表示session数目已经超出指定端口所允许的最大值。
注意：
*********************************************/
...
/********************************************
功能：删除指定的PIM Session
输入：ulSessionId： PIM Session ID，由函数PimAddV2Session输出；
输出：无
返回：成功则返回FD_OK；失败则返回如下值：
	  FD_ERROR_START_PROTOCOL_EMULATION表示正在进行协议仿真，
	  PIM_ERROR_SESSION_ID表示输入的ulSessionId错误。
注意：
*********************************************/
...
返回：成功则返回FD_OK；失败则返回如下值：
	  FD_ERROR_START_PROTOCOL_EMULATION表示正在进行协议仿真，
	  PIM_ERROR_SESSION_ID表示输入的ulSessionId错误，
	  PIM_ERROR_EXCEED_MAX_GROUP_COUNT表示组地址数已经超出指定端口所允许的最大值，
...
/********************************************
功能：删除指定的组地址池
输入：ulGroupPoolId： 组地址池ID，由函数PimAddGroupPool输出；
输出：无
返回：成功则返回FD_OK；失败则返回如下值：
	  FD_ERROR_START_PROTOCOL_EMULATION表示正在进行协议仿真，
	  PIM_ERROR_GROUP_POOL_ID表示输入的ulGroupPoolId错误。
注意：
*********************************************/
...
/********************************************
功能：设置指定端口发送PIM Membership Report报文的速率
输入：ulChsId： 机框ID，1-based；
      ulSlotId：槽位号，1-based;
	  ulPortId：端口号，1-based;
	  ulPPS：   发送PIM Membership Report报文的速率，单位为Packets/Sec，取值范围为1--10000；
输出：无
返回：成功则返回FD_OK；失败则返回如下值：
	  FD_ERROR_START_PROTOCOL_EMULATION表示正在进行协议仿真，
      FD_ERROR_CHASSIS_ID表示输入的ulChsId错误，
